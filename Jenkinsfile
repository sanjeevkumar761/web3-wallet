...
export serviceLBIP=$(kubectl get services web3test-helm-chart  --output jsonpath=\'{.status.loadBalancer.ingress[0].ip}\')
npx localtunnel --port 4000 --local-host $serviceLBIP > localtunnel.log &
cat ./localtunnel.log
'''
      }
    }
