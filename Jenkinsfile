pipeline {
  ...
  stages {
    stage('deploy') {
      steps {
        sh '''ls ./kubeconfig
docker build --tag web3wallet:${env.BUILD_NUMBER} .
docker tag web3wallet:${env.BUILD_NUMBER} saneevkumar761/web3wallet:latest
docker login docker.io -u ${DOCKERHUB_USER} -p ${DOCKERHUB_PASSWORD}
docker push saneevkumar761/web3wallet:${env.BUILD_NUMBER}'''
        sh 'kubectl get all'
      }
    }
